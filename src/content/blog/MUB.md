...
Let $\mathcal{B}_1 = \{|\phi_1\rangle, \dots, |\phi_d\rangle\}$ and $\mathcal{B}_2 = \{|\psi_1\rangle, \dots, |\psi_d\rangle\}$ be two orthonormal bases in the $d$-dimensional state space. They are said to be **mutually unbiased bases (MUB)** if and only if $$\left|\langle\phi_i | \psi_j\rangle\right| = \frac{1}{\sqrt{d}}$$ for every $i, j = 1, \dots, d$. A set $\{\mathcal{B}_1, \dots, \mathcal{B}_m\}$ of orthonormal bases in $\mathbb{C}^d$ is called a <i>set of mutually unbiased bases</i> (a set of <span class="define-term" data-def="Let the system be prepared in state |φᵢ⟩ from basis ℬ₁, and measured in ℬ₂. The probability of an outcome is given by the mod sq of the inner product: P(ψⱼ|φᵢ) = |⟨ψⱼ|φᵢ⟩|². For these bases to be mutually unbiased, each outcome must be equally likely. Therefore: P(ψ₁|φᵢ) = P(ψ₂|φᵢ) = ... = p. Since all probabilities must sum to 1, ΣP(ψⱼ|φᵢ) = 1, which means dp = 1. This gives us the final result: p = 1/d">MUB</span>) if each pair of bases $\mathcal{B}_i$ and $\mathcal{B}_j$ are mutually unbiased.
<br><br>
